// =================================================================
// Advanced Analytics Bot - v144.3 (Final Sanitization Fix)
// =================================================================
// --- IMPORTS ---
...
// =================================================================
// SECTION 1: OKX API ADAPTER & CACHING (Unchanged)
// =================================================================
...
// =================================================================
// SECTION 2: DATABASE & HELPER FUNCTIONS (Unchanged)
// =================================================================
...
const sendDebugMessage = async (message) => {
    ...
    if (settings.debugMode) {
        try {
            ...
            await bot.telegram.sendMessage(AUTHORIZED_USER_ID, `.debugLine *Debug \\(OKX\\):* ${sanitizedMessage}`, { parse_mode: "MarkdownV2" });
        } catch (e) {
...
// =================================================================
// SECTION 3: FORMATTING AND MESSAGE FUNCTIONS (Sanitization Fix)
// =================================================================
...
function formatPrivateBuy(details) { 
    ...
    msg += `━━━━━━━━━━━━━━━━━━━━\n*بتاريخ:* ${sanitizeMarkdownV2(new Date().toISOString().slice(0, 19).replace('T', ' '))}`;
    return msg; 
}
...
function formatPrivateCloseReport(details) {
    ...
    msg += `━━━━━━━━━━━━━━━━━━━━\n*بتاريخ الإغلاق:* ${sanitizeMarkdownV2(new Date().toISOString().slice(0, 19).replace('T', ' '))}`;
    return msg;
}
...
async function formatPortfolioMsg(assets, total, capital) {
    ...
    caption += `*بتاريخ: ${sanitizeMarkdownV2(new Date().toISOString().slice(0, 19).replace('T', ' '))}*\n`;
...
async function formatAdvancedMarketAnalysis(ownedAssets = []) {
    ...
    let msg = `🚀 *تحليل السوق المتقدم \\(OKX\\)* \\| ${sanitizeMarkdownV2(new Date().toISOString().slice(0, 10))}\n`;
    msg += `*تفاصيل السوق:* ${totalCount} أصول، ${gainersCount} مربحة، ${losersCount} خسارة (${gainersPercent.toFixed(2)}% مربحة، ${losersPercent.toFixed(2)}% خسارة)\n`;
    msg += `*الخاترة:* ${breadthConclusion}\n`;
    msg += ```\n`;
    msg += `*أعلى مربحة 5 أصول:*\n`;
    topGainers.forEach(g => msg += `  \\- ${g.instId}: \`+${g.change24h.toFixed(2)}%\`\n`);
    msg += `*أدنى خسارة 5 أصول:*\n`;
    topLosers.forEach(l => msg += `  \\- ${l.instId}: \`-${l.change24h.toFixed(2)}%\`\n`);
    msg += `*5 أصول بحجم أعلى:*\n`;
    highVolume.forEach(h => msg += `  \\- ${h.instId}: \`+${h.change24h.toFixed(2)}%\`\n`);
    msg += ```\n`
    ...
    if (ownedAssets.length > 0) {
        ...
        ownedAssets.forEach(asset => {
            const priceData = prices[`${asset.asset}-USDT`] || { price: asset.avgBuyPrice, change24h: 0 };
            const currentPrice = priceData.price;
            const positionPnl = (currentPrice - asset.avgBuyPrice) * asset.amount;
            const positionPnlPercent = asset.avgBuyPrice > 0 ? (positionPnl / (asset.avgBuyPrice * asset.amount)) * 100 : 0;
            const positionPnlEmoji = positionPnl >= 0 ? '🟢' : '🔴';
            msg += `  \\- ${asset.asset}: \`+${positionPnlPercent.toFixed(2)}%\` (${positionPnlEmoji})\n`;
        });
        msg += ```\n`;
    }
...
async function formatQuickStats(assets, total, capital) { 
    ...
    msg += `\n\n⏰ *آخر تحديث:* ${sanitizeMarkdownV2(new Date().toISOString().slice(0, 19).replace('T', ' '))}`;
    return msg;
}
...
// =================================================================
// SECTION 4: DATA PROCESSING & AI ANALYSIS (Unchanged)
// =================================================================
...
// =================================================================
// SECTION 5: BACKGROUND JOBS & DYNAMIC MANAGEMENT (Unchanged logic)
// =================================================================
...
// =================================================================
// SECTION 7: BOT HANDLERS (Telegraf Conversion)
// =================================================================
...
// --- Main Menu Handlers using bot.hears for clarity ---
...
bot.hears("🧮 حاسبة الربح والخسارة", (ctx) => ctx.reply("✍️ لحساب الربح/الخسارة، استخدم أمر `/pnl` بالصيغة التالية:\n`/pnl <سعر الشراء> <سعر البيع> <الكمية>`", {parse_mode: "MarkdownV2"}));
...
// =================================================================
// SECTION 8: HANDLER LOGIC (Refactored into smaller functions)
// =================================================================
...
async function handleReviewTradesRequest(ctx) {
    ...
    try {
        ...
        if (closedTrades.length === 0) {
            await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, "ℹ️ لا يوجد سجل صفقات مغلقة \\(متوافقة\\) لمراجعتها.", { parse_mode: "MarkdownV2" });
            return;
        }
        const keyboard = Markup.inlineKeyboard(
            closedTrades.map(trade => {
                // Use asset and date instead of price to avoid unescaped periods
                const closedDate = trade.closedAt ? new Date(trade.closedAt).toISOString().slice(0, 10) : 'غير محدد';
                return [Markup.button.callback(`${trade.asset} | أغلق بتاريخ ${closedDate}`, `review_trade_${trade._id}`)];
            })
        );
...
async function handlePublishAction(ctx) {
    ...
    if (reportMarkerStart !== -1) {
        ...
        if (data === "publish_report" && reportMarkerEnd !== -1) {
            ...
            await ctx.editMessageText(newText, { parse_mode: 'MarkdownV2' });
        } else {
    ...
    try {
        if (data === "add_virtual_trade") {
            ...
            if (activeTrades.length === 0) {
                await ctx.editMessageText("✅ لا توجد توصيات افتراضية نشطة حاليًا.", { reply_markup: virtualTradeKeyboard.reply_markup });
                return;
            }
            const prices = await getCachedMarketPrices();
...
async function handleSettingsActions(ctx) {
    ...
    try {
        switch(data) {
            case "set_capital": waitingState = 'set_capital'; await ctx.editMessageText("💰 يرجى إرسال المبلغ الجديد لرأس المال \\(رقم فقط\\).", { parse_mode: "MarkdownV2" }); break;
            case "back_to_settings": await sendSettingsMenu(ctx); break;
            case "manage_movement_alerts": await sendMovementAlertsMenu(ctx); break;
            case "set_global_alert": waitingState = 'set_global_alert_state'; await ctx.editMessageText("✍️ يرجى إرسال النسبة العامة الجديدة \\(مثال: `5`\\).", { parse_mode: "MarkdownV2" }); break;
...
async function startBot() {
    try {
        ...
        await bot.telegram.sendMessage(AUTHORIZED_USER_ID, "✅ *تم إعادة تشغيل البوت بنجاح \\(v144\\.3 \\- Final Sanitization Fix\\)*\n\n\\- تم إصلاح جميع مشاكل تنسيق النصوص\\.", { parse_mode: "MarkdownV2" }).catch(console.error);
...
// =================================================================
// SECTION 10: WEBSOCKET MANAGER (Unchanged)
// =================================================================
