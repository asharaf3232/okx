// =================================================================
// Advanced Analytics Bot - v135.1 (Stable with Closed Trade Review)
        ...
        case "🔍 مراجعة الصفقات":
            ...
            try {
                ...
                if (closedTrades.length === 0) {
                    await ctx.telegram.editMessageText(loadingMsgReview.chat.id, loadingMsgReview.message_id, undefined, "ℹ️ لا يوجد سجل صفقات مغلقة لمراجعتها.");
                    return;
                }
                ...
                await ctx.telegram.editMessageText(loadingMsgReview.chat.id, loadingMsgReview.message_id, undefined, "👇 *اختر صفقة من القائمة أدناه لمراجعتها:*", {
                    parse_mode: "Markdown",
                    reply_markup: Markup.inlineKeyboard(tradeButtons)
                });
            } catch (e) {
                console.error("Error in 'مراجعة الصفقات':", e);
                await ctx.telegram.editMessageText(loadingMsgReview.chat.id, loadingMsgReview.message_id, undefined, `❌ حدث خطأ: ${e.message}`);
            }
            break;
        case "💡 توصية افتراضية":
