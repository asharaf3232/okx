...
// --- رسائل القناة العامة (يجب أن تكون دقيقة جداً في التهريب) ---
function formatPublicBuy(d) {
    const jId = sanitizeMarkdownV2(d.journeyId || 'N/A');
    const size = sanitizeMarkdownV2(formatNumber(d.oldTotalValue > 0 ? (d.tradeValue / d.oldTotalValue) * 100 : 0));
...
function formatPublicClose(d) {
    ...
    // ملاحظة: النصوص الثابتة هنا تم وضع تهريب لها يدوياً للنقاط
