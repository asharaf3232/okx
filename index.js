// =================================================================
// Advanced Analytics Bot - v143.7 (Movement Alert Fix)
// =================================================================
// --- IMPORTS ---
...
async function checkPriceMovements() {
    try {
        ...
        const oldPriceTracker = await loadPriceTracker(); // Load the state from the last run
        const prices = await getCachedMarketPrices();
        ...
        // Initialize a new tracker for the current state
        const newPriceTracker = {
            totalPortfolioValue: currentTotalValue,
            assets: {}
        };

        // If it's the first run, just save the current state and don't send alerts
        ...
        // --- Check individual asset movements ---
        for (const asset of assets) {
            if (asset.asset === 'USDT' || !asset.price) continue;
            
            // Add current price to the new tracker that will be saved later
            newPriceTracker.assets[asset.asset] = asset.price;

            // Compare with the price from the PREVIOUS run
        ...
        // --- Check total portfolio movement ---
        ...
        // After all checks are done, save the new state for the next run.
        await savePriceTracker(newPriceTracker);
