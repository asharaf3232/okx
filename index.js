// =================================================================
// Advanced Analytics Bot - v143.7 (Comprehensive Markdown Sanitization Fix)
// =================================================================
// --- IMPORTS ---
...
const sanitizeMarkdownV2 = (text) => {
    ...
    // Note: '.', '!', '-' are escaped, but should not be escaped inside code blocks (`...`)
...
function formatClosedTradeReview(trade, currentPrice) {
    ...
    msg += `  \\- 💵 *سعر الشراء الأصلي:* \`$${formatSmart(avgBuyPrice)}\`\n`;
    msg += `  \\- ✅ *سعر الإغلاق الفعلي:* \`$${formatSmart(avgSellPrice)}\`\n`;
    msg += `  \\- 📈 *السعر الحالي للسوق:* \`$${formatSmart(currentPrice)}\`\n\n`;
...
function formatPrivateBuy(details) {
    ...
    msg += `━━━━━━━━━━━━━━━━━━━━\n*تحليل الصفقة:*\n`;
    msg += ` ▪️ **سعر التنفيذ:** \`$${formatSmart(price)}\`\n`;
...
function formatPrivateCloseReport(details) {
    ...
    msg += ` ▪️ **متوسط سعر الدخول:** \`$${formatSmart(avgBuyPrice)}\`\n`;
    msg += ` ▪️ **متوسط سعر الخروج:** \`$${formatSmart(avgSellPrice)}\`\n`;
    msg += ` ▪️ **أعلى قمة سعرية مسجلة:** \`$${formatSmart(highestPrice)}\`\n`;
    msg += ` ▪️ **أدنى قاع سعري مسجل:** \`$${formatSmart(lowestPrice)}\`\n`;
...
function formatPublicBuy(details) {
    ...
    msg += `*سعر الدخول الحالي:* \`$${formatSmart(price)}\`\n`;
...
function formatPublicSell(details) {
    ...
    msg += `*الأصل:* \`${asset}/USDT\`\n`;
    msg += `*سعر البيع الجزئي:* \`$${formatSmart(price)}\`\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━\n*استراتيجية إدارة المحفظة:*\n`;
...
async function formatPortfolioMsg(assets, total, capital) {
    ...
    if (cryptoAssets.length > 0) {
        ...
        caption += `━━━━━━━━━━━━━━━━━━━\n*🎯 أبرز تحركات اليوم:*\n`;
        caption += `▫️ *الأفضل أداءً:* 🟢 ${sanitizeMarkdownV2(bestPerformer.asset)} \\(\`+${formatNumber(bestPerformer.change24h * 100)}%\`\\)\n`;
        if (cryptoAssets.length > 1) {
            caption += `▫️ *الأقل أداءً:* 🔴 ${sanitizeMarkdownV2(worstPerformer.asset)} \\(\`${formatNumber(worstPerformer.change24h * 100)}%\`\\)\n`;
        }
    }
    ...
    displayAssets.forEach((a, index) => {
        ...
        if (position?.avgBuyPrice) { caption += `├─ *متوسط الشراء:* \`$${formatSmart(position.avgBuyPrice)}\`\n`; }
        caption += `├─ *سعر السوق:* \`$${formatSmart(a.price)}\`\n`;
...
async function formatQuickStats(assets, total, capital) {
    ...
    if (cryptoAssets.length === 0) {
        ...
        cryptoAssets.forEach((asset, index) => {
            ...
            if (extremes) {
                msg += ` *الأسبوعي:* قمة \`$${formatSmart(extremes.weekly.high)}\` / قاع \`$${formatSmart(extremes.weekly.low)}\`\n`;
                msg += ` *الشهري:* قمة \`$${formatSmart(extremes.monthly.high)}\` / قاع \`$${formatSmart(extremes.monthly.low)}\`\n`;
                msg += ` *السنوي:* قمة \`$${formatSmart(extremes.yearly.high)}\` / قاع \`$${formatSmart(extremes.yearly.low)}\`\n`;
                msg += ` *التاريخي:* قمة \`$${formatSmart(extremes.allTime.high)}\` / قاع \`$${formatSmart(extremes.allTime.low)}\``;
            } else {
...
async function generateAndSendCumulativeReport(ctx, asset) {
    try {
        ...
        if (trades.length === 0) {
            await ctx.reply(`ℹ️ لا يوجد سجل صفقات مغلقة لعملة *${sanitizeMarkdownV2(asset)}*\\.`, { parse_mode: "MarkdownV2" });
            return;
        }
        ...
        report += `تداولاتك في *${sanitizeMarkdownV2(asset)}* أضافت ما قيمته \`${impactSign}${formatNumber(totalPnl)}\` ${impactEmoji} إلى محفظتك بشكل تراكمي\\.\n\n`;
...
async function handleCallbackQuery(ctx, data) {
    try {
        ...
        if (data.startsWith("review_trade_")) {
            ...
            await ctx.editMessageText(`⏳ جاري تحليل صفقة \`${tradeId.substring(0, 8)}... \``, { parse_mode: "MarkdownV2"});
    ...
    try {
        switch (state) {
            case 'ai_ask_coin':
                ...
                const loading = await ctx.reply(`🧠 جاري تحليل عملة ${sanitizeMarkdownV2(coin)}\\.\\.\\.`);
            ...
            case 'set_capital':
                ...
                if (!isNaN(amount) && amount >= 0) {
                    ...
                    await ctx.reply(`✅ *تم تحديث رأس المال إلى:* \`$${formatNumber(amount)}\``, { parse_mode: "MarkdownV2" });
                } else {
            ...
            case 'set_global_alert_state':
                ...
                if (!isNaN(percent) && percent > 0) {
                    ...
                    await ctx.reply(`✅ تم تحديث النسبة العامة لتنبيهات الحركة إلى \`${percent}%\`\\.`, { parse_mode: "MarkdownV2"});
                } else {
            ...
            case 'set_coin_alert_state':
                ...
                if (coinPercent === 0) {
                    ...
                    await ctx.reply(`✅ تم تحديث النسبة المخصصة لـ *${sanitizeMarkdownV2(symbol_coin_alert.toUpperCase())}* إلى \`${coinPercent}%\`\\.`, { parse_mode: "MarkdownV2"});
                }
            ...
            case 'set_alert':
                ...
                await ctx.reply(`✅ تم ضبط التنبيه: ${sanitizeMarkdownV2(symbol.toUpperCase())} ${sanitizeMarkdownV2(cond)} ${price}`, { parse_mode: "MarkdownV2" });
                break;
...
async function startBot() {
    ...
    try {
        ...
        await runDailyJobs();
        connectToOKXSocket();
        await bot.api.sendMessage(AUTHORIZED_USER_ID, "✅ *تم إعادة تشغيل البوت بنجاح \\(v143\\.7 \\- Comprehensive Markdown Fix\\)*\n\n\\- تم إجراء إصلاح شامل لجميع مشاكل تنسيق الرسائل لضمان عمل كافة التقارير بشكل صحيح\\.", { parse_mode: "MarkdownV2" }).catch(console.error);
    } catch (e) {
