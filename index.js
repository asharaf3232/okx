// =================================================================
// Advanced Analytics Bot - v145.1 (Comprehensive Sanitization Fix)
// =================================================================
// --- IMPORTS ---
...
async function formatPortfolioMsg(assets, total, capital) {
    ...
    if (totalValue24hAgo > 0) {
        ...
        dailyPnlText = ` ${dailyEmoji} \`$${sanitizeMarkdownV2(dailySign)}${sanitizeMarkdownV2(formatNumber(dailyPnl))}\` ${sanitizeMarkdownV2(`(${dailySign}${formatNumber(dailyPnlPercent)}%)`)}`;
    }
    ...
    caption += ` ▫️ *إجمالي الربح غير المحقق:* ${pnlEmoji} \`$${sanitizeMarkdownV2(pnlSign)}${sanitizeMarkdownV2(formatNumber(pnl))}\` ${sanitizeMarkdownV2(`(${pnlSign}${formatNumber(pnlPercent)}%)`)}\n`;
    ...
    displayAssets.forEach((a, index) => {
        ...
        caption += `├─ *القيمة الحالية:* \`$${sanitizeMarkdownV2(formatNumber(a.value))}\` ${sanitizeMarkdownV2(`(*الوزن:* ${formatNumber(percent)}%)`)}\n`;
        ...
        if (position?.avgBuyPrice > 0) {
            ...
            caption += `╰─ *ربح/خسارة غير محقق:* ${assetPnlEmoji} \`$${sanitizeMarkdownV2(assetPnlSign)}${sanitizeMarkdownV2(formatNumber(assetPnl))}\` ${sanitizeMarkdownV2(`(${assetPnlSign}${formatNumber(assetPnlPercent)}%)`)}`;
        } else {
    ...
    caption += `\n\n━━━━━━━━━━━━━━━━━━━━\n*USDT (الرصيد النقدي)* 💵\n`;
    caption += `*القيمة:* \`$${sanitizeMarkdownV2(formatNumber(usdtAsset.value))}\` ${sanitizeMarkdownV2(`(*الوزن:* ${formatNumber(cashPercent)}%)`)}`;
    return { caption };
}
async function formatAdvancedMarketAnalysis(ownedAssets = []) {
    const prices = await getCachedMarketPrices();
    if (!prices || prices.error) return sanitizeMarkdownV2(`❌ فشل جلب بيانات السوق. ${prices.error || ''}`);
    ...
    msg += `${sanitizeMarkdownV2(smartRecommendation)}`;
    return msg;
}
...
async function getAIAnalysisForAsset(asset) {
    ...
    **1. بيانات المشروع الأساسية:**
    - لم يتم العثور على بيانات أساسية محدثة للمشروع.
        `;
    }
    ...
    const basePrompt = `
    أنت محلل خبير. قم بتحليل عملة ${asset} بشكل شامل.
    ${fundamentalSection}
    ...
    **المطلوب:**
    قدم تحليلًا متكاملاً في فقرة واحدة. ابدأ بوصف المشروع، ثم اربطه بالوضع الفني، وأخيرًا، قدم توصية واضحة (شراء/بيع/مراقبة) مع الأخذ في الاعتبار تاريخي الشخصي مع العملة.
    `;
    ...
    try {
        ...
        if (data.status !== 'ok') {
            if (data.code === 'apiKeyInvalid' || data.code === 'apiKeyMissing') {
                 throw new Error("مفتاح NewsAPI غير صالح أو مفقود.");
            }
        ...
        for (const asset of allAssets) {
            ...
            if (analysisResult.type === 'buy') {
                ...
                if (settings.autoPostToChannel) {
                    ...
                    const confirmationMessage = `${privateMessage.replace(/\.\s*$/, '')}\n\n*هل تود نشر الملخص في القناة؟*${hiddenMarker}`;
                    await sendMessageSafely(AUTHORIZED_USER_ID, confirmationMessage, confirmationKeyboard);
                }
            }
        }
        if (stateNeedsUpdate) {
        ...
        for (const asset of assets) {
            ...
            if (lastPrice) {
                ...
                if (Math.abs(changePercent) >= threshold) {
                    ...
                    await bot.telegram.sendMessage(AUTHORIZED_USER_ID, message, { parse_mode: "MarkdownV2" });
...
const mainKeyboard = Markup.keyboard([
    ...
    ["🔍 مراجعة الصفقات", "🧠 تحليل بالذكاء الاصطناعي"],
...
const aiKeyboard = Markup.inlineKeyboard([
    [Markup.button.callback("💼 تحليل المحفظة", "ai_analyze_portfolio"), Markup.button.callback("🪙 تحليل عملة", "ai_analyze_coin")],
    [Markup.button.callback("📰 أخبار عامة", "ai_get_general_news"), Markup.button.callback("📈 أخبار محفظتي", "ai_get_portfolio_news")]
]);
...
async function sendMovementAlertsMenu(ctx) { 
    ...
    await ctx.editMessageText(sanitizeMarkdownV2(text), { parse_mode: "MarkdownV2", ...keyboard }); 
}
...
bot.start((ctx) => {
    ...
    ctx.reply(sanitizeMarkdownV2(welcomeMessage), { parse_mode: "MarkdownV2", ...mainKeyboard });
});
...
bot.command("pnl", async (ctx) => { 
    ...
    if (args.length !== 3) { 
        return await ctx.reply( sanitizeMarkdownV2(`❌ *صيغة غير صحيحة.*\n*مثال:* \`/pnl <سعر الشراء> <سعر البيع> <الكمية>\`\n\n*مثلاً: /pnl 100 120 50*`), { parse_mode: "MarkdownV2" } ); 
    } 
    ...
    if (isNaN(buyPrice) || isNaN(sellPrice) || isNaN(quantity) || buyPrice <= 0 || sellPrice <= 0 || quantity <= 0) { 
        return await ctx.reply(sanitizeMarkdownV2("❌ *خطأ:* تأكد من أن جميع القيم هي أرقام موجبة وصحيحة.")); 
    } 
    ...
    const sign = pnl >= 0 ? '+' : ''; 
    const msg = `🧮 *نتيجة حساب الربح والخسارة*\n\n` + ` ▪️ *إجمالي تكلفة الشراء:* \`$${sanitizeMarkdownV2(formatNumber(investment))}\`\n` + ` ▪️ *إجمالي قيمة البيع:* \`$${sanitizeMarkdownV2(formatNumber(saleValue))}\`\n` + `━━━━━━━━━━━━━━━━━━━━\n` + `*صافي الربح/الخسارة:* \`${sanitizeMarkdownV2(sign)}${sanitizeMarkdownV2(formatNumber(pnl))}\` ${sanitizeMarkdownV2(`(${sign}${formatNumber(pnlPercent)}%)`)}\n` + `**الحالة النهائية: ${status}**`; 
    await ctx.reply(msg, { parse_mode: "MarkdownV2" }); 
});

// --- Main Menu Handlers ---
bot.hears("📊 عرض المحفظة", handlePortfolioRequest);
bot.hears("🚀 تحليل السوق", handleMarketAnalysisRequest);
bot.hears("🔍 مراجعة الصفقات", handleReviewTradesRequest);
bot.hears("💡 توصية افتراضية", (ctx) => ctx.reply("اختر الإجراء المطلوب للتوصيات الافتراضية:", virtualTradeKeyboard));
bot.hears("⚡ إحصائيات سريعة", handleQuickStatsRequest);
bot.hears("📈 أداء المحفظة", (ctx) => {
    const performanceKeyboard = Markup.inlineKeyboard([
        [Markup.button.callback("آخر 24 ساعة", "chart_24h"), Markup.button.callback("آخر 7 أيام", "chart_7d"), Markup.button.callback("آخر 30 يومًا", "chart_30d")]
    ]);
    ctx.reply("اختر الفترة الزمنية لعرض تقرير الأداء:", performanceKeyboard);
});
bot.hears("📈 تحليل تراكمي", (ctx) => {
    waitingState = 'cumulative_analysis_asset';
    ctx.reply(sanitizeMarkdownV2("✍️ يرجى إرسال رمز العملة التي تود تحليلها (مثال: `BTC`)."), { parse_mode: "MarkdownV2" });
});
bot.hears("🧠 تحليل بالذكاء الاصطناعي", (ctx) => ctx.reply("اختر نوع التحليل الذي تريده:", aiKeyboard));
bot.hears("🧮 حاسبة الربح والخسارة", (ctx) => ctx.reply(sanitizeMarkdownV2("✍️ لحساب الربح/الخسارة، استخدم أمر `/pnl` بالصيغة التالية:\n`/pnl <سعر الشراء> <سعر البيع> <الكمية>`"), {parse_mode: "MarkdownV2"}));
bot.hears("⚙️ الإعدادات", sendSettingsMenu);
...
// =================================================================
// SECTION 8: HANDLER LOGIC
// =================================================================

async function handlePortfolioRequest(ctx) {
    const loadingMessage = await ctx.reply("⏳ جاري إعداد التقرير...");
    try {
        const prices = await getCachedMarketPrices();
        ...
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}

async function handleMarketAnalysisRequest(ctx) {
    const loadingMessage = await ctx.reply("⏳ جاري تحليل السوق...");
    try {
        ...
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, marketMsg, { parse_mode: "MarkdownV2" });
    } catch (e) {
        console.error(`Error in handleMarketAnalysisRequest:`, e);
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}

async function handleReviewTradesRequest(ctx) {
    const loadingMessage = await ctx.reply("⏳ جارٍ جلب أحدث 5 صفقات مغلقة...");
    try {
        const closedTrades = await getCollection("tradeHistory").find({ quantity: { $exists: true } }).sort({ closedAt: -1 }).limit(5).toArray();
        if (closedTrades.length === 0) {
            await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, sanitizeMarkdownV2("ℹ️ لا يوجد سجل صفقات مغلقة (متوافقة) لمراجعتها."), { parse_mode: "MarkdownV2" });
            return;
        }
        const keyboard = Markup.inlineKeyboard(
            closedTrades.map(trade => [
                Markup.button.callback(`${trade.asset} | ${new Date(trade.closedAt).toLocaleDateString('ar-EG')}`, `review_trade_${trade._id}`)
            ])
        );
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, "*👇 اختر صفقة من القائمة أدناه لمراجعتها:*", { parse_mode: "MarkdownV2", ...keyboard });
    } catch (e) {
        console.error(`Error in handleReviewTradesRequest:`, e);
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}

async function handleQuickStatsRequest(ctx) {
    const loadingMessage = await ctx.reply("⏳ جاري حساب الإحصائيات...");
    try {
        ...
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, quickStatsMsg, { parse_mode: "MarkdownV2" });
    } catch (e) {
        console.error(`Error in handleQuickStatsRequest:`, e);
        await ctx.telegram.editMessageText(ctx.chat.id, loadingMessage.message_id, undefined, sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}
...
async function handleAiActions(ctx) {
    ...
    try {
        if (data === "ai_get_general_news") {
        ...
        } else if (data === "ai_get_portfolio_news") {
        ...
        } else if (data === "ai_analyze_portfolio") {
            await ctx.editMessageText("🧠 جاري طلب تحليل المحفظة من الذكاء الاصطناعي...");
            const prices = await getCachedMarketPrices();
            if (!prices || prices.error) throw new Error("فشل جلب بيانات السوق.");
            const capital = await loadCapital();
            ...
            await ctx.editMessageText(`*🧠 تحليل الذكاء الاصطناعي - المحفظة*\n\n${sanitizeMarkdownV2(aiResponse)}`, { parse_mode: "MarkdownV2" });
        } else if (data === "ai_analyze_coin") {
            waitingState = "ai_ask_coin";
            await ctx.editMessageText(sanitizeMarkdownV2("✍️ أرسل رمز العملة التي ترغب في تحليلها (مثل BTC)."), { parse_mode: "MarkdownV2" });
        }
    } catch (e) {
        console.error(`Error in handleAiActions for "${data}":`, e);
        await ctx.editMessageText(sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}

async function handleReviewTradeAction(ctx) {
    await ctx.answerCbQuery();
    const tradeId = ctx.match[0].split('_')[2];
    ...
    try {
        ...
        if (!trade || !trade.quantity) throw new Error("لم يتم العثور على الصفقة أو أنها لا تحتوي على بيانات الكمية اللازمة للتحليل.");
        const prices = await getCachedMarketPrices();
        const currentPrice = prices[`${trade.asset}-USDT`]?.price;
        if (!currentPrice) throw new Error(`تعذر جلب السعر الحالي لعملة ${trade.asset}.`);
        ...
        console.error(`Error in handleReviewTradeAction for ID "${tradeId}":`, e);
        await ctx.editMessageText(sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}
...
async function handleChartAction(ctx) {
    ...
    try {
        ...
        if (!portfolioHistory || portfolioHistory.length < 2) throw new Error("لا توجد بيانات كافية لهذه الفترة.");
        ...
        await ctx.replyWithPhoto({ url: report.chartUrl }, { caption: report.caption, parse_mode: "MarkdownV2" });
...
async function handlePublishAction(ctx) {
    await ctx.answerCbQuery();
    const data = ctx.match[0];
    ...
    if (reportMarkerStart !== -1) {
        ...
        if (data === "publish_report" && reportMarkerEnd !== -1) {
            ...
            await bot.telegram.sendMessage(TARGET_CHANNEL_ID, reportContent, { parse_mode: "MarkdownV2" });
            const newText = privatePart.replace('*هل تود نشر الملخص في القناة؟*', '✅ *تم نشر التقرير بنجاح في القناة.*');
            await ctx.editMessageText(sanitizeMarkdownV2(newText), { parse_mode: 'MarkdownV2' });
        } else {
            const newText = privatePart.replace('*هل تود نشر الملخص في القناة؟*', '❌ *تم تجاهل نشر التقرير.*');
            await ctx.editMessageText(sanitizeMarkdownV2(newText), { parse_mode: 'MarkdownV2' });
        }
    }
}

async function handleVirtualTradeActions(ctx) {
    await ctx.answerCbQuery();
    const data = ctx.match[0];
    try {
        if (data === "add_virtual_trade") {
            waitingState = 'add_virtual_trade';
            await ctx.editMessageText(sanitizeMarkdownV2("✍️ *لإضافة توصية افتراضية، أرسل التفاصيل في 5 أسطر منفصلة:*\n\n`BTC-USDT`\n`65000` (سعر الدخول)\n`70000` (سعر الهدف)\n`62000` (وقف الخسارة)\n`1000` (المبلغ الافتراضي)\n\n**ملاحظة:** *لا تكتب كلمات مثل 'دخول' أو 'هدف'، فقط الأرقام والرمز.*"), { parse_mode: "MarkdownV2" });
        } else if (data === "track_virtual_trades") {
            await ctx.editMessageText("⏳ جاري جلب التوصيات النشطة...");
            const activeTrades = await getActiveVirtualTrades();
            if (activeTrades.length === 0) {
                await ctx.editMessageText(sanitizeMarkdownV2("✅ لا توجد توصيات افتراضية نشطة حاليًا."), { reply_markup: virtualTradeKeyboard.reply_markup });
                return;
            }
            const prices = await getCachedMarketPrices();
            if (!prices || prices.error) throw new Error("فشل جلب الأسعار، لا يمكن متابعة التوصيات.");
            ...
            for (const trade of activeTrades) {
                ...
                if (!currentPrice) {
                    ...
                    const sign = pnl >= 0 ? '+' : '';
                    const emoji = pnl >= 0 ? '🟢' : '�';
                    reportMsg += `*${sanitizeMarkdownV2(trade.instId)}* ${emoji}\n` + ` ▫️ *الدخول:* \`$${sanitizeMarkdownV2(formatSmart(trade.entryPrice))}\`\n` + ` ▫️ *الحالي:* \`$${sanitizeMarkdownV2(formatSmart(currentPrice))}\`\n` + ` ▫️ *ربح/خسارة:* \`${sanitizeMarkdownV2(sign)}${sanitizeMarkdownV2(formatNumber(pnl))}\` ${sanitizeMarkdownV2(`(${sign}${formatNumber(pnlPercent)}%)`)}\n` + ` ▫️ *الهدف:* \`$${sanitizeMarkdownV2(formatSmart(trade.targetPrice))}\`\n` + ` ▫️ *الوقف:* \`$${sanitizeMarkdownV2(formatSmart(trade.stopLossPrice))}\`\n`;
                }
                reportMsg += "━━━━━━━━━━━━━━━━━━━━\n";
            }
            await ctx.editMessageText(reportMsg, { parse_mode: "MarkdownV2", reply_markup: virtualTradeKeyboard.reply_markup });
        }
    } catch (e) {
        console.error(`Error in handleVirtualTradeActions for "${data}":`, e);
        await ctx.editMessageText(sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}

async function handleSettingsActions(ctx) {
    await ctx.answerCbQuery();
    const data = ctx.match[0];
    try {
        switch(data) {
            case "set_capital": waitingState = 'set_capital'; await ctx.editMessageText(sanitizeMarkdownV2("💰 يرجى إرسال المبلغ الجديد لرأس المال (رقم فقط)."), { parse_mode: "MarkdownV2" }); break;
            case "back_to_settings": await sendSettingsMenu(ctx); break;
            case "manage_movement_alerts": await sendMovementAlertsMenu(ctx); break;
            case "set_global_alert": waitingState = 'set_global_alert_state'; await ctx.editMessageText(sanitizeMarkdownV2("✍️ يرجى إرسال النسبة العامة الجديدة (مثال: `5`)."), { parse_mode: "MarkdownV2" }); break;
            case "set_coin_alert": waitingState = 'set_coin_alert_state'; await ctx.editMessageText(sanitizeMarkdownV2("✍️ يرجى إرسال رمز العملة والنسبة.\n*مثال:*\n`BTC 2.5`"), { parse_mode: "MarkdownV2" }); break;
            case "view_positions": const positions = await loadPositions(); if (Object.keys(positions).length === 0) { await ctx.editMessageText(sanitizeMarkdownV2("ℹ️ لا توجد مراكز مفتوحة."), { reply_markup: Markup.inlineKeyboard([Markup.button.callback("🔙 العودة للإعدادات", "back_to_settings")]).reply_markup }); break; } let posMsg = "📄 *قائمة المراكز المفتوحة:*\n"; for (const symbol in positions) { const pos = positions[symbol]; posMsg += `\n- *${sanitizeMarkdownV2(symbol)}:* متوسط الشراء \`$${sanitizeMarkdownV2(formatSmart(pos.avgBuyPrice))}\``; } await ctx.editMessageText(posMsg, { parse_mode: "MarkdownV2", reply_markup: Markup.inlineKeyboard([Markup.button.callback("🔙 العودة للإعدادات", "back_to_settings")]).reply_markup }); break;
            case "delete_alert": const alerts = await loadAlerts(); if (alerts.length === 0) { await ctx.editMessageText(sanitizeMarkdownV2("ℹ️ لا توجد تنبيهات مسجلة."), { reply_markup: Markup.inlineKeyboard([Markup.button.callback("🔙 العودة للإعدادات", "back_to_settings")]).reply_markup }); break; } let alertMsg = "🗑️ *اختر التنبيه لحذفه:*\n\n"; alerts.forEach((alert, i) => { alertMsg += `*${i + 1}.* \`${sanitizeMarkdownV2(alert.instId)} ${sanitizeMarkdownV2(alert.condition)} ${sanitizeMarkdownV2(alert.price)}\`\n`; }); alertMsg += "\n*أرسل رقم التنبيه الذي تود حذفه.*"; waitingState = 'delete_alert_number'; await ctx.editMessageText(sanitizeMarkdownV2(alertMsg), { parse_mode: "MarkdownV2" }); break;
            ...
            case "delete_all_data": waitingState = 'confirm_delete_all'; await ctx.editMessageText(sanitizeMarkdownV2("⚠️ *تحذير: هذا الإجراء لا يمكن التراجع عنه!* لحذف كل شيء، أرسل: `تأكيد الحذف`"), { parse_mode: "MarkdownV2" }); break;
        }
    } catch (e) {
        console.error(`Error in handleSettingsActions for "${data}":`, e);
        await ctx.editMessageText(sanitizeMarkdownV2(`❌ حدث خطأ: ${e.message}`), { parse_mode: "MarkdownV2"});
    }
}
    ...
    try {
        switch (state) {
            case 'ai_ask_coin':
                ...
                await ctx.telegram.editMessageText(ctx.chat.id, loading.message_id, undefined, `*🧠 تحليل الذكاء الاصطناعي | ${sanitizeMarkdownV2(coin)}*\n\n${sanitizeMarkdownV2(aiResponse)}`, { parse_mode: "MarkdownV2" });
                break;
            ...
            case 'set_capital':
                ...
                if (!isNaN(amount) && amount >= 0) {
                    ...
                    await ctx.reply(sanitizeMarkdownV2("❌ مبلغ غير صالح."));
                }
                break;
            case 'set_global_alert_state':
                ...
                if (!isNaN(percent) && percent > 0) {
                    ...
                    await ctx.reply(sanitizeMarkdownV2("❌ *خطأ:* النسبة يجب أن تكون رقمًا موجبًا."));
                }
                break;
            case 'set_coin_alert_state':
                const parts_coin_alert = text.split(/\s+/);
                if (parts_coin_alert.length !== 2) {
                    await ctx.reply(sanitizeMarkdownV2("❌ *صيغة غير صحيحة*. يرجى إرسال رمز العملة ثم النسبة."), { parse_mode: "MarkdownV2"});
                    return;
                }
                ...
                if (isNaN(coinPercent) || coinPercent < 0) {
                    await ctx.reply(sanitizeMarkdownV2("❌ *خطأ:* النسبة يجب أن تكون رقمًا."));
                    return;
                }
            ...
            case 'confirm_delete_all':
                if (text === 'تأكيد الحذف') {
                    ...
                    await ctx.reply(sanitizeMarkdownV2("✅ تم حذف جميع بياناتك."));
                } else {
                    await ctx.reply(sanitizeMarkdownV2("❌ تم إلغاء الحذف."));
                }
                break;
            case 'set_alert':
                const parts_alert = text.trim().split(/\s+/);
                if (parts_alert.length !== 3) {
                    await ctx.reply(sanitizeMarkdownV2("❌ صيغة غير صحيحة. مثال: `BTC > 50000`"), { parse_mode: "MarkdownV2"});
                    return;
                }
                const [symbol, cond, priceStr] = parts_alert;
                if (cond !== '>' && cond !== '<') {
                    await ctx.reply(sanitizeMarkdownV2("❌ الشرط غير صالح. استخدم `>` أو `<`."), { parse_mode: "MarkdownV2"});
                    return;
                }
                const price = parseFloat(priceStr);
                if (isNaN(price) || price <= 0) {
                    await ctx.reply(sanitizeMarkdownV2("❌ السعر غير صالح."));
                    return;
                }
            ...
            case 'delete_alert_number':
                ...
                if (isNaN(index) || index < 0 || index >= currentAlerts.length) {
                    await ctx.reply(sanitizeMarkdownV2("❌ رقم غير صالح."));
                    return;
                }
                currentAlerts.splice(index, 1);
                await saveAlerts(currentAlerts);
                await ctx.reply(sanitizeMarkdownV2(`✅ تم حذف التنبيه.`));
                break;
        }
    } catch (e) {
        console.error(`Error in handleWaitingState for state "${state}":`, e);
        await ctx.reply(sanitizeMarkdownV2(`❌ حدث خطأ أثناء معالجة إدخالك. يرجى المحاولة مرة أخرى.`), { parse_mode: "MarkdownV2"});
    }
}

// =================================================================
// SECTION 9: SERVER AND BOT INITIALIZATION
// =================================================================
...
async function startBot() {
    try {
        ...
        await bot.telegram.sendMessage(AUTHORIZED_USER_ID, sanitizeMarkdownV2("✅ *تم إعادة تشغيل البوت بنجاح (v145.0 - Final Fix)*\n\n- تم إصلاح جميع مشاكل تنسيق النصوص لضمان استقرار كامل."), { parse_mode: "MarkdownV2" }).catch(console.error);
