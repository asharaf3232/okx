// =================================================================
// Advanced Analytics Bot - v135.0 (Closed Trade Review Feature)
...
class OKXAdapter {
    ...
    async getBalanceForComparison() {
        try {
            ...
            const balances = {};
            json.data[0].details.forEach(asset => { const amount = parseFloat(asset.eq); if (amount > 0) balances[asset.ccy] = amount; });
            return balances;
        } catch (e) {
...
// =================================================================
// SECTION 2: DATA PROCESSING FUNCTIONS (Your Original Code)
// =================================================================
...
// =================================================================
// SECTION 3: FORMATTING AND MESSAGE FUNCTIONS (Your Original Code)
// =================================================================
...
function formatClosedTradeReview(trade, currentPrice) {
    ...
    msg += `  -  बाजार *السعر الحالي للسوق:* \`$${formatNumber(currentPrice, 4)}\`\n\n`;
...
// MODIFIED: Added a new button for reviewing closed trades
const mainKeyboard = Markup.keyboard([
    ['📊 عرض المحفظة', '📈 أداء المحفظة'],
    ['🚀 تحليل السوق', '💡 توصية افتراضية'],
    ['⚡ إحصائيات سريعة', '📈 تحليل تراكمي'],
    ['🔍 مراجعة الصفقات', 'ℹ️ معلومات عملة'],
    ['🔔 ضبط تنبيه', '🧮 حاسبة الربح والخسارة'],
    ['⚙️ الإعدادات']
]).resize();
...
async function sendSettingsMenu(ctx) {
    ...
    try {
        if (ctx.callbackQuery) {
            await ctx.editMessageText(text, { parse_mode: "Markdown", reply_markup: settingsKeyboard.reply_markup });
        } else {
...
async function sendMovementAlertsMenu(ctx) {
    ...
    const keyboard = Markup.inlineKeyboard([
        Markup.button.callback("📊 تعديل النسبة العامة", "set_global_alert"),
        Markup.button.callback("💎 تعديل نسبة عملة", "set_coin_alert"),
...
bot.on("callback_query", async (ctx) => {
    ...
    try {
        if (data.startsWith("chart_")) {
            ...
            if (period === '24h') {
                history = await loadHourlyHistory(); periodLabel = "آخر 24 ساعة"; bar = '1H'; limit = 24;
            } else if (period === '7d') {
                history = await loadHistory(); periodLabel = "آخر 7 أيام"; bar = '1D'; limit = 7;
            } else if (period === '30d') {
                history = await loadHistory(); periodLabel = "آخر 30 يومًا"; bar = '1D'; limit = 30;
            } else { return; }
            ...
            if (report.error) {
                ...
                await ctx.replyWithPhoto({url: report.chartUrl}, { caption: report.caption, parse_mode: "Markdown" });
                await ctx.deleteMessage().catch(console.error);
            }
            return;
        }

        // --- NEW: Handle closed trade review requests ---
            ...
            if (reportMarkerStart !== -1) {
                ...
                if (data === "publish_report") {
                    if (reportMarkerEnd !== -1) {
                        ...
                        await bot.telegram.sendMessage(process.env.TARGET_CHANNEL_ID, reportContent, { parse_mode: "Markdown" });
        ...
        switch (data) {
            ...
            case "view_positions": const positions = await loadPositions(); if (Object.keys(positions).length === 0) { await ctx.editMessageText("ℹ️ لا توجد مراكز مفتوحة.", { reply_markup: Markup.inlineKeyboard([Markup.button.callback("🔙 العودة للإعدادات", "back_to_settings")]) }); break; } let posMsg = "📄 *قائمة المراكز المفتوحة:*\n"; for (const symbol in positions) { const pos = positions[symbol]; posMsg += `\n- *${symbol}:* متوسط الشراء \`$${formatNumber(pos.avgBuyPrice, 4)}\``; } await ctx.editMessageText(posMsg, { parse_mode: "Markdown", reply_markup: Markup.inlineKeyboard([Markup.button.callback("🔙 العودة للإعدادات", "back_to_settings")]) }); break;
            case "delete_alert": const alerts = await loadAlerts(); if (alerts.length === 0) { await ctx.editMessageText("ℹ️ لا توجد تنبيهات مسجلة.", { reply_markup: Markup.inlineKeyboard([Markup.button.callback("🔙 العودة للإعدادات", "back_to_settings")]) }); break; } let alertMsg = "🗑️ *اختر التنبيه لحذفه:*\n\n"; alerts.forEach((alert, i) => { alertMsg += `*${i + 1}.* \`${alert.instId} ${alert.condition} ${alert.price}\`\n`; }); alertMsg += "\n*أرسل رقم التنبيه الذي تود حذفه.*"; waitingState = 'delete_alert_number'; await ctx.editMessageText(alertMsg, { parse_mode: "Markdown" }); break;
        ...
        switch (state) {
            ...
            case 'coin_info':
                ...
                try {
                    ...
                    await ctx.telegram.editMessageText(loadingMsg.chat.id, loadingMsg.message_id, undefined, msg, { parse_mode: "Markdown" });
                } catch(e) {
                    console.error("Error fetching coin info:", e);
                    await ctx.telegram.editMessageText(loadingMsg.chat.id, loadingMsg.message_id, undefined, `❌ حدث خطأ أثناء جلب البيانات: ${e.message}`);
                }
                return;
            case 'set_alert':
    ...
    switch (text) {
        case "📊 عرض المحفظة":
            ...
            try {
                ...
                await ctx.telegram.editMessageText(loadingMsgPortfolio.chat.id, loadingMsgPortfolio.message_id, undefined, caption, { parse_mode: "Markdown" });
            } catch (e) {
                console.error("Error in 'عرض المحفظة':", e);
                await ctx.telegram.editMessageText(loadingMsgPortfolio.chat.id, loadingMsgPortfolio.message_id, undefined, `❌ حدث خطأ: ${e.message}`);
            }
            break;
        case "🚀 تحليل السوق":
            ...
            try {
                ...
                await ctx.telegram.editMessageText(loadingMsgMarket.chat.id, loadingMsgMarket.message_id, undefined, marketMsg, { parse_mode: "Markdown" });
            } catch (e) {
                console.error("Error in 'تحليل السوق':", e);
                await ctx.telegram.editMessageText(loadingMsgMarket.chat.id, loadingMsgMarket.message_id, undefined, `❌ حدث خطأ أثناء تحليل السوق: ${e.message}`);
            }
            break;
        // --- NEW: Handle the new button press ---
        case "🔍 مراجعة الصفقات":
            await ctx.reply("⏳ جارٍ جلب أحدث 5 صفقات مغلقة...");
            try {
                const closedTrades = await getCollection("tradeHistory").find({}).sort({ closedAt: -1 }).limit(5).toArray();
                if (closedTrades.length === 0) {
                    await ctx.reply("ℹ️ لا يوجد سجل صفقات مغلقة لمراجعتها.");
                    return;
                }
                const tradeButtons = closedTrades.map(trade => [
                    Markup.button.callback(
                        `${trade.asset} | أغلق بسعر $${formatNumber(trade.avgSellPrice, 4)}`,
                        `review_trade_${trade._id}`
                    )
                ]);
                await ctx.reply("👇 *اختر صفقة من القائمة أدناه لمراجعتها:*", {
                    parse_mode: "Markdown",
                    reply_markup: Markup.inlineKeyboard(tradeButtons)
                });
            } catch (e) {
        ...
        case "⚡ إحصائيات سريعة":
            ...
            try {
                ...
                await ctx.telegram.editMessageText(loadingMsgQuick.chat.id, loadingMsgQuick.message_id, undefined, quickStatsMsg, { parse_mode: "Markdown" });
            } catch (e) {
                console.error("Error in 'إحصائيات سريعة':", e);
                await ctx.telegram.editMessageText(loadingMsgQuick.chat.id, loadingMsgQuick.message_id, undefined, `❌ حدث خطأ: ${e.message}`);
            }
            break;
        case "📈 أداء المحفظة":
...
async function startBot() {
    try {
        ...
        await bot.telegram.sendMessage(AUTHORIZED_USER_ID, "✅ *تم إعادة تشغيل البوت بنجاح*\n\nتم تفعيل المراقبة المتقدمة لمنصة OKX.", {parse_mode: "Markdown"}).catch(console.error);
